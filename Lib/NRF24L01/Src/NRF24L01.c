...
uint8_t NRF24L01_Write_Reg(uint8_t reg,uint8_t value)
{
	uint8_t status;	
  	...
  	status =SPI1_ReadWriteByte(&hspi1,reg);//发送寄存器号 
  	SPI1_ReadWriteByte(&hspi1,value);      //写入寄存器的值
...
uint8_t NRF24L01_Read_Reg(uint8_t reg)
{
	  uint8_t reg_val;	    
   	NRF24L01_SPI_CS_ENABLE();              //使能SPI传输		
  	SPI1_ReadWriteByte(&hspi1,reg);    //发送寄存器号
  	reg_val=SPI1_ReadWriteByte(&hspi1,0XFF);//读取寄存器内容
  	NRF24L01_SPI_CS_DISABLE();            //禁止SPI传输		    
...
uint8_t NRF24L01_Read_Buf(uint8_t reg,uint8_t *pBuf,uint8_t len)
{
	uint8_t status,uint8_t_ctr;	       
  	NRF24L01_SPI_CS_ENABLE();            //使能SPI传输
  	status=SPI1_ReadWriteByte(&hspi1,reg);//发送寄存器值(位置),并读取状态值   	   
	for(uint8_t_ctr=0;uint8_t_ctr<len;uint8_t_ctr++)pBuf[uint8_t_ctr]=SPI1_ReadWriteByte(&hspi1,0XFF);//读出数据
...
uint8_t NRF24L01_Write_Buf(uint8_t reg, uint8_t *pBuf, uint8_t len)
{
	...
	NRF24L01_SPI_CS_ENABLE();              //使能SPI传输
  	status = SPI1_ReadWriteByte(&hspi1,reg);//发送寄存器值(位置),并读取状态值
  	for(uint8_t_ctr=0; uint8_t_ctr<len; uint8_t_ctr++)SPI1_ReadWriteByte(&hspi1,*pBuf++); //写入数据	 
  	NRF24L01_SPI_CS_DISABLE();            //关闭SPI传输
  	return status;              //返回读到的状态值
}				   
//启动NRF24L01发送一次数据
